...
public class WebInteractionAnalyzer {
    ...
    /**
     * Infers response propagation constraint. (from SQL response to web response)
     *
     * @param samples SQLSamples for the interaction signature.
     * @return a set of response propagation constraints.
     */
    public static Set<PropagationPathConstraint> inferResponsePropagatePath(List<SQLSample> samples) {
        ...
        // figures out which attributes from SQL view corresponds to which variables from web response.
        Set<UserOwnershipConstraint> consts = InferenceEngine.inferMembershipConstraint(sqlRespSamples,
                webRespSamples, InferenceEngine.MODE_SUBSTRING_MATCH);
        ...
        for (String xpath : constsByXpath.keySet()) {
            ...
            // for each web response variable with same XPath, merge if they all propagated from a same attribute
            // from SQL view.
            if (srcVars.size() > 1) {
    ...
    public static Set<PropagationPathConstraint> inferParameterPropagatePath(List<SQLSample> samples) {
        ...
        for (EqualityConstraint equalityConst : equalityConsts) {
            PropagationPathConstraint constraint =
                    new PropagationPathConstraint(equalityConst.getLeftSideVariable(), equalityConst.getRightSideVariable());
            constraints.add(constraint);
        }
