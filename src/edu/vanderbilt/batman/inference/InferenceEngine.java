...
public class InferenceEngine {
    ...
    public static Set<UserSpecificVariableConstraint> inferSpecificValueConstraint(List<Set<Variable>> samples) {
        ...
        if (samples.size() < 2)
            return constraints;    // need at least two samples (e.g., users) for differential analysis.
		...
		*/
        ...
        // each sample is a list of variables with consistent values for a user.
        for (int i = 0; i < samples.size(); i++) {
            ...
            for (Variable var : sample) {
                ...
                // only add unique values for each variable that already has consistent values.
